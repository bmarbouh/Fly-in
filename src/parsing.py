...
class Parsing:
    ...
    def parse_zones(self, line: str):
        ...
        if len(zone_data) > 3:
            metadata = self.parse_metadata(zone_data[3], ["color", "max_drones", "zone"])
        
        zone = Zone(name, x, y, metadata.get("color", "red"), int(metadata.get("max_drones", 1)), metadata.get("zone", "normal"))
